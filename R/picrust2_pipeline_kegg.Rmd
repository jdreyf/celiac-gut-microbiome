---
title: "Function Prediction (KEGG Pathways)"
author: "Hui/Jonathan"
...
pathway_pipeline.py -i /n/data1/joslin/cores/bbcore/celiac_disease_16sn/picrust2/picrust2_out_pipeline/KO_metagenome_out/pred_metagenome_unstrat.tsv.gz -o KEGG_pathways_out --no_regroup --map /home/hp135/picrust2/picrust2/default_files/pathway_mapfiles/KEGG_pathways_to_KO.tsv -p 4
...
## Annotate pathways
```{r annot}
annot <- read.delim("B:/annotations/kegg/pathway_id/KEGG_pathways_info.tsv.gz", row.names = 1, header = FALSE)
...
signif_hist(mtt, name = "kegg_signif_hist", pi0 = TRUE)
write.csv(mtt.df, "kegg_pwys_stats.csv", na = "")
```
...
The histograms of significance are at [kegg_signif_hist.pdf](./kegg_signif_hist.pdf). If no KO terms were associated with the phenotype, we would expect the p-value histogram to be flat and all FDRs to be near one. The more associated KO terms there are, the more enrichment there is at low p-values, the lower will be the FDRs.  We also estimate the proportion of the true null hypothesis (i.e. non-significant KO terms) [@langaas_2005].    
KO term statistics tables for all KO terms [kegg_pwys_stats.csv](./kegg_pwys_stats.csv). The table contains the average log2 abundance of each group, p-values, FDR, log fold-change, fold-change, and pathway information.      

## Plots
```{r plots}
## boxplot
top_pwy1 <- select.top(mtt.df, contrasts.v = contr.v[grep("Presort", names(contr.v))])
pdf("top_kegg_pwys_boxplot_presort.pdf", 9, 3)
for (pwy in top_pwy1){
...
pdf("top_kegg_pwys_boxplot_postsort.pdf", 9, 5)
for (pwy in top_pwy2){
    dat2p <- data.frame(logabun = pwys.tab[pwy, md$Sort != "Presort"], md[md$Sort != "Presort", ])
...
ezheat(pwys.tab[top_pwy1, md$Sort == "Presort"], pheno.df = md.df1,  sc = "z", reorder_rows = TRUE, 
       ...
       gaps_col = gaps.col1, main = "Log2 abundance", name = "top_kegg_pwys_heat_presort", height = 8, width = 16, clip = 2)
...
ezheat(pwys.tab[top_pwy2, md$Sort != "Presort"], pheno.df = md.df2,  sc = "z", reorder_rows = TRUE, 
       ...
       gaps_col = gaps.col2, main = "Log2 abundance", name = "top_kegg_pwys_heat_postsort", height = 8, width = 30, clip = 2)
```

Equal numbers of top KO terms (based on p-values) are selected from each comparison. The boxplots for top KO terms are at [top_kegg_pwys_boxplot_presort.pdf](./top_kegg_pwys_boxplot_presort.pdf), and [top_kegg_pwys_boxplot_postsort.pdf](./top_kegg_pwys_boxplot_postsort.pdf). The same sets of top KO terms are use in heatmaps [top_kegg_pwys_heat_presort.pdf](./top_kegg_pwys_heat_presort.pdf) and [top_kegg_pwys_heat_postsort.pdf](./top_kegg_pwys_heat_postsort.pdf).


```{r check}
stopifnot(rownames(pwys.tab) %in% rownames(annot))
stopifnot(colnames(pwys.tab) == rownames(md))
stopifnot(rownames(mtt.df)[1] == "ko00643")
